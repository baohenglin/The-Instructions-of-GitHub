    ...
    常见问题:
    (1)如何建立与"README.md"同级的文件夹?
    点击“Create new file”，然后输入文件夹名称，此时在输入的名称后面加上"/"，然后再输入下一级文档的名称进行创建保存。
    注意一定要要在文件夹名称后面加上‘/’，否则创建出来的不是文件夹。这样就可以在该文件夹路径下上传.md的文档了。
    (2)删除“repositories”中的某个仓库之后,该仓库对应的commit记录还会保留吗?
    答:不会保留。
